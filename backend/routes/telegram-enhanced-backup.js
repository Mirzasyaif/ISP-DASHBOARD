        ...
        bot.on('callback_query', async (callbackQuery) => {
            ...
            try {
                if (data.startsWith('confirm_payment:')) {
                    ...
                    if (success) {
                        bot.sendMessage(chatId, 
                            `✅ Pembayaran untuk ${username} berhasil dicatat.\n` +
                        ...
                        bot.sendMessage(chatId, `❌ Gagal mencatat pembayaran untuk ${username}.`);
                    }
                    
                    // Answer callback query
                    bot.answerCallbackQuery(callbackQuery.id, { text: '✅ Pembayaran dicatat' });
                    ...
                    bot.answerCallbackQuery(callbackQuery.id, { text: 'Masukkan jumlah pembayaran' });
                    ...
                    bot.answerCallbackQuery(callbackQuery.id, { text: `Memilih ${username}` });
                    ...
                    if (user) {
                        await processPaymentConfirmation(chatId, user, fromUser);
                        bot.answerCallbackQuery(callbackQuery.id, { text: `Memilih ${user.pppoe_username}` });
                    } else {
                        bot.sendMessage(chatId, `❌ User dengan ID ${clientId} tidak ditemukan.`);
                        bot.answerCallbackQuery(callbackQuery.id, { text: 'User tidak ditemukan' });
                    }
